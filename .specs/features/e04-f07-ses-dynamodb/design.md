...
| `AWSSDK.SimpleEmail` (legacy V1), not `AWSSDK.SimpleEmailV2` | `IAmazonSimpleEmailService.SendEmailAsync` (V1 `Source`/`Destination`/`Message` shape) | Discovered during T07 execution: LocalStack Community edition doesn't implement the `sesv2` API at all ("pro feature" per LocalStack's own error) — using V2 would mean `SesEmailSender` could never be exercised against a real emulator, only mocks, breaking AD-013's automated-tests-use-LocalStack guarantee for this one adapter. V1 is fully supported by LocalStack Community. SES (even LocalStack's V1 emulation) also requires the sender identity to be verified before `SendEmailAsync` succeeds — the test fixture gained a `VerifySenderAsync` helper for this. |
