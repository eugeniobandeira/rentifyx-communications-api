...
Next up overall: **E-04 is fully complete** — all local/remote feature branches (`feat/e04-f07-ses-dynamodb`, `feat/e04-f08-throttling`, `feat/e04-f09-reliability`) cleaned up 2026-07-15. **Planned (not started):** `.specs/features/tech-debt-scaffold-cleanup/` (spec+design+tasks written 2026-07-15) — removes the entire `Example` scaffold (EF Core/SQL Server included) across all 5 layers + 6 test projects, deletes now-unused generic repo interfaces (`IAddRepository<T>` etc.)/`PagedResult<T>`/`IUnitOfWork`, and cleans up config reads: `Kafka`/`Resilience`/`Reconciliation`/`SecretsProvider` get real `IOptions<T>` (they back DI-constructed singletons/hosted services); `Cors`/`RateLimit`/`OpenApi` get a plain typed record bound once inside their own startup extension method, no `IOptions<T>`/DI registration; `AWS` is untouched (`GetAWSOptions()` already binds it) (T01-T10). User confirmed the first 2 scope questions, then refined the 3rd (2026-07-15): not everything needs `IOptions<T>` — only where a DI-constructed class actually consumes it via constructor injection; this rule is now codified in CLAUDE.md's new "Configuration Binding" section. Ready for Execute whenever picked up. This also resolves two long-standing STATE.md todos below (`CustomWebApplicationFactory` EF gap, "Examples scaffold doesn't match architecture"). After that: marketing campaigns (E-07) and identity-api integration contract (E-08), both spec'd but Execute not started.
