...
**Last Updated:** 2026-07-14T13:30:00Z
...
- [ ] PR #7's F-07 file layout doesn't match CLAUDE.md's folder-segmentation convention: `DynamoDbNotificationRepository`/`DynamoDbConsentRepository` live directly under `Repositories/` (not a `Repositories/Notifications/` subfolder) with mapping done via private `ToItem`/`FromItem` methods inline in each repository class, instead of dedicated static mapper classes. The abandoned local redo on `feat/e04-ses-dynamodb-local-attempt` did this the CLAUDE.md way (`Repositories/Notifications/` + `NotificationItemMapper`/`ConsentItemMapper`) — worth cherry-picking that structural pattern as a small follow-up refactor if this bothers future readers, but not urgent since the current code is correct and tested. Discovered 2026-07-14 during the branch-divergence reconciliation.
