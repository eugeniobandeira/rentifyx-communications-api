...
**Current Work:** **E-01, E-02, and E-03 are all DONE.** E-01 merged via PR #2 (T16 branch protection applied, `enforce_admins` later disabled — see Todos). E-02 merged via PR #4, then retrofit for naming conventions via PR #5 (`Notification` → `NotificationEntity`, interfaces moved into `Domain/Interfaces/Notifications/`). E-03 (Application Layer — Use Cases) implemented 2026-07-13 on branch `feat/e03-application-layer` (not yet merged): `DispatchNotificationRequest`/`DispatchOutcome`, `DispatchNotificationValidator`, `DispatchNotificationHandler` (full outbox sequence: validate → construct → idempotency → consent → dispatch → render → persist Dispatching → send → mark final), IoC registration, and `NotificationRequestedConsumer` wired to actually process messages (was a no-op skeleton since E-01) — deserializes via `System.Text.Json`, resolves the `Scoped` handler via a per-message `IServiceScopeFactory` scope, always commits the offset even on failure (confirmed 2026-07-13 — no DLQ exists until E-04, so blocking the partition was rejected as worse). 46 unit tests passing across `Tests.Domain`(29)/`Tests.Validators`(+7)/`Tests.Handlers`(+11)/`Tests.Api`(+5). Next up: **E-04 (Infrastructure — SES, DynamoDB, Idempotency & Resilience)** per `.specs/project/ROADMAP.md` — needs Specify phase; this is where `INotificationRepository`/`IConsentRepository`/`ITemplateRenderer`/`IEmailSender` get real implementations, and where the enum-as-string persistence todo below gets resolved. Per AD-013, LocalStack scope was narrowed back to automated tests only (T12 rework) — manual dev run keeps using real AWS. Scope also includes marketing campaigns (E-07) and identity-api integration contract (E-08), both spec'd (E-07 also has design+tasks) but Execute not started — both depend on E-04 landing.
...
- [ ] `CustomWebApplicationFactory` (`03-tests/05-Integration`) is broken in three compounding, dormant ways — found during a 2026-07-13 general review, not caused by any T0x task: (1) it calls `builder.UseEnvironment("Testing")` but the config file is named `appsettings.Test.json`, not `appsettings.Testing.json`, so it's never actually loaded; (2) even if the name were fixed, `InfrastructureDependencyInjection.AddDbContext` unconditionally calls `UseSqlServer(...)` regardless of environment, so the `"DefaultConnection": "InMemory"` value in that file wouldn't do anything useful; (3) `Microsoft.EntityFrameworkCore.InMemory` isn't referenced anywhere, so there's no EF InMemory provider to switch to even if the plumbing were fixed. Currently harmless because its only consumer, `ExampleEndpointTests.GetAll_ShouldReturnOk`, is `[Fact(Skip = "Not yet implemented")]`. Not fixed — needs a design decision (EF InMemory provider vs. a real Testcontainers SQL Server, matching the LocalStack pattern T12 used for AWS) before whoever un-skips that test, or before this pattern gets copied for E-02+ handler/endpoint tests.
- [ ] E-04 (DynamoDB implementation): `Channel` and `NotificationStatus` enums must be persisted as their **string name** (e.g. `"Sent"`), never as the underlying `int` — user explicitly rejected numeric enum storage on 2026-07-13 ("fica sem sentido"). Domain keeps them as C# enums (type-safety); this only constrains `DynamoDbNotificationRepository`'s serialization in E-04 — use `.ToString()` or an explicit `IPropertyConverter`, not the default numeric marshalling.
- [ ] The `Examples` scaffold's persistence layer (`AppDbContext`, `ExampleRepository`, EF Core + SQL Server) doesn't match the project's actual, already-decided architecture: notifications persist to **DynamoDB** (AD-008, single-table design `PK=NOTIF#{correlationId}` / `GSI1=RECIPIENT#{recipientId}` / `GSI2=ID#{id}` — corrected 2026-07-13, see AD-008 — atomic idempotency via conditional `PutItem`), per `docs/architecture/overview.md` and ROADMAP.md E-04. Found during the 2026-07-13 review while answering a direct question about which database to use. The SQL Server/EF Core scaffold is template boilerplate that predates that decision — expect it to be replaced, not extended, once E-02/E-04 land real repositories; the `Microsoft.EntityFrameworkCore.SqlServer` package likely gets removed from Infrastructure at that point too, unless a second, non-DynamoDB need for it turns up.
